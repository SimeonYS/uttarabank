...
class UuttarabankPipeline:

    # Database setup
    conn = sqlite3.connect('uttarabank.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `uttarabank`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM uttarabank WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO uttarabank (date, title, link, content)"
